...
HPI="../target/alaudaDevops-client.hpi"
...
cp -f "$HPI" jpi/alaudaDevops-client.jpi
if [ "$?" != "0" ]; then
    echo "Unable to copy $HPI"
    exit 1
fi

docker build -f ./Dockerfile-jenkins-test-new-plugin -t alaudaDevops/jenkins-client-plugin-snapshot-test:latest .
if [ "$?" != "0" ]; then
